...
const message = {
    ...
    terminal: {
        ...
        pinSessionHelper:
            'После закрепления сессия продолжает работать на сервере {0} минут после того, как вы покинете страницу или закроете браузер.',
        sessionRestored: 'Восстановлено сессий: {0}',
        sessionExpired: 'Сессия истекла или больше не существует, подключитесь заново.',
        sessionKicked: 'Эта сессия открыта в другом окне.',
        sessionLimit: 'Достигнуто максимальное количество закреплённых сессий.',
        closePinnedConfirm:
            'Этот терминал закреплён. При закрытии сессия на сервере также будет завершена. Продолжить?',
        sessionKeepAlive: 'Время хранения сессии (минуты)',
        sessionKeepAliveHelper:
            'Сколько времени сохраняется закреплённая сессия терминала после ухода со страницы. 0 отключает эту функцию.',
        sessionMaxPinned: 'Максимум закреплённых сессий',
        sessionBuffer: 'Буфер воспроизведения (КБ)',
    },
    toolbox: {
        common: {
    ...
    license: {
        ...
        vllm: 'Обновите до коммерческой версии, чтобы централизованно управлять сервисами vLLM в 1Panel: развертыванием, настройкой, жизненным циклом и отслеживанием задач.',
        'model-downloader':
    ...
    firewall: {
        ...
        plan_overlapping_rule_with_different_action:
            'Есть пересекающееся правило с противоположным действием. Операция остановлена.',
        plan_partially_overlapping_rule_with_different_action:
            'Правило частично пересекается с правилом противоположного действия. Результат зависит от порядка правил.',
        ...
        resetWhitelistRulesHelper:
            'Очистить активную пользовательскую конфигурацию {0}, восстановить исходные настройки и отключить {0}; политики базы данных сохраняются и могут быть синхронизированы повторно.',
        cleanupForwardingBackendHelper:
            'Удалить из {0} все правила и цепочки переадресации портов 1Panel, сохранив только данные в базе данных',
        cleanupDockerBackendHelper:
            'Удалить из {0} все правила и цепочки защиты портов Docker 1Panel, сохранив только данные в базе данных',
        cleanupBeforeBackendSwitch:
            'Текущий бэкенд {0} всё ещё содержит правила времени выполнения 1Panel. Очистите его перед переключением на {1}.',
        cleanupAction: 'Очистить',
        backendSwitchNotice:
            'Перед переключением системного брандмауэра, переадресации портов или защиты Docker очистите текущий бэкенд. Политики базы данных сохраняются и могут быть инициализированы или синхронизированы после переключения.',
        switchBackendHelper: 'Переключиться на {0}?',
        ruleSyncTitle: 'Синхронизация правил',
        ruleSyncAction: 'Синхронизировать правила',
        ruleSyncHelper:
            'Синхронизирует правила 1Panel из выбранного брандмауэра с текущим. Исходные правила не удаляются, а внешние правила не синхронизируются.',
        ruleSyncDatabase: 'База данных 1Panel',
        ruleSyncDatabaseHelper:
            'Синхронизирует и приводит управляемые правила текущего межсетевого экрана в соответствие с правилами из базы данных 1Panel. Недостающие правила будут добавлены, а лишние — удалены.',
        ...
        externallyExposed: 'Разрешить внешний доступ (0.0.0.0 / ::)',
        restrictedBinding: 'Запретить внешний доступ (127.0.0.1 / ::1)',
        ...
        user: {
            ...
            auth: {
                ...
                saml2: {
                    ...
                    nameAttributeHelper:
                        'Считывает этот атрибут из проверенного SAML Assertion и сопоставляет его с именем пользователя 1Panel.',
                    ...
                    syncLogoutHelper: 'Выход из 1Panel или IdP также завершает сеанс на другой стороне.',
                    testBeforeBrowserTest: 'Проверьте текущую конфигурацию перед запуском проверки входа в браузер.',
                    testBeforeEnable:
