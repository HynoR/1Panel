...
const message = {
    ...
    firewall: {
        ...
        modeManaged: '1Panel 全权管理',
        modeManagedTip: '1Panel 全权管理本机的 iptables 规则。',
        modeExternal: '管理 ufw（不介入启停）',
        ...
        snapshotTag: '标签',
        snapshotRestoreHelper: '恢复将重建 1Panel 防火墙链并开启确认窗口——若被锁外且未确认会自动回退。是否继续？',
        family: 'IP 版本',
        ...
        portHelper1: '如 8080，多个 8080,8081，范围 8080-8089',
        strategy: '策略',
        accept: '允许',
        drop: '拒绝',
        anyWhere: '所有 IP',
        ...
        addressHelper1: '如 172.16.10.11 或 172.16.0.0/24，多个用逗号隔开',
        allIP: '所有 IP',
        portRule: '端口规则',
        forwardRule: '端口转发',
        ipRule: 'IP 规则',
        ...
        targetIP: '目标 IP',
        targetPort: '目标端口',
        forwardEmpty: '还没有端口转发规则。可把外部端口的流量转发到内网其它 IP / 端口。',
        ...
        importInvalidSkipped: '{0} 行格式无效已跳过',
        ...
        source: '来源',
        ruleObjectPort: '按端口',
        ruleObjectAddress: '按来源 IP',
        advancedOptions: '高级选项',
        goOverviewInit: '防火墙尚未就绪，请前往「概览」启动并初始化。',
        goOverviewStart: '前往概览启动',
        rescueChannel: '保底端口（不会被锁外）',
        rescueAllowed: '已放行',
        rescueClosable: '可关闭',
        rescueReadOnly: '保底端口（只读）',
        defaultPolicy: '默认入站策略',
        policyLoose: '宽松模式（未列出端口默认放行）',
        policyRecommended: '推荐',
        policyStrict: '白名单模式（未列出端口默认拒绝）',
        wizardStrictWarn: '仅列出的端口可访问，其余全部拒绝，配置不当可能把你锁在服务器外。',
        whitelistMode: '白名单模式',
        whitelistModeTip:
        ...
        flowSectionTitle: '入站评估顺序',
        flowSectionHint: '自上而下匹配，命中即停止，未命中的走默认策略。',
        flowInbound: '入站',
        flowRescue: '保底端口',
        flowDeny: '封禁列表',
        flowBaseline: '保底端口',
        flowAllow: '放行列表',
        flowDefaultLoose: '默认：宽松模式',
        flowDefaultStrict: '默认：白名单模式',
        flowInboundTip: '入站流量按下面顺序自上而下匹配，命中即停止。',
        flowRescueTip: '紧急放行你当前的连接，避免误封后被锁在外面。',
        flowDenyTip: '封禁列表优先级最高，命中即拒绝。',
        flowBaselineTip: '始终放行 SSH、面板等保底端口，不可删除或改为屏蔽。',
        flowAllowTip: '你手动开放的放行规则。',
        flowDefaultLooseTip: '未匹配任何规则的入站流量默认放行。',
        flowDefaultStrictTip: '未匹配任何规则的入站流量默认拒绝。',
        strategyTip: '放行＝允许访问；屏蔽＝拒绝访问。SSH、面板等保底端口始终受保护。',
        hintAllowPort: '放行后，匹配的端口将允许入站访问；未填写来源 IP 时对所有来源开放。',
        hintDenyPort: '屏蔽后，匹配的端口将拒绝入站访问。SSH、面板等保底端口受保护，不会被误锁。',
        hintAllowAddress: '放行后，来自该 IP / IP 段的入站访问将被允许。',
        hintDenyAddress: '屏蔽后，来自该 IP / IP 段的所有入站访问将被拒绝。',
        levelDeny: '封禁列表',
        levelBaseline: '保底端口',
        levelAllow: '放行列表',
        noListen: '无监听',
        riskTitle: '这条规则可能让你失联',
        riskBlockSelf: '此规则可能拦截你当前的连接，应用后若失联，60 秒内未确认将自动撤销。',
        riskAck: '我已了解风险',
        riskProceed: '仍要应用',
        redlineTitle: '操作已拒绝',
        redlineBlockBoth: '该规则会同时阻断 SSH 与 1Panel 面板，已拒绝。如需默认拒绝入站，请使用「白名单模式」开关。',
        redlineHint: '如需放通请改用「放行」，或把封禁范围缩小到具体 IP 段。',
        applying: '正在应用防火墙变更…',
        autoReverted: '变更未确认，已自动撤销',
        wizardStep1: '确认保底端口',
        wizardStep2: '选默认策略',
        wizardStep3: '应用并自检',
        enableManaged: '启用 1Panel 管理',
        disableManaged: '停用 1Panel 管理',
        requiredPorts: '保底端口（不可删除）',
        editablePorts: '可编辑端口',
        deleteRescuePortHelper: '删除 {0} 后该端口将不再随防火墙自动放行，可能影响外部访问，是否继续？',
        snapshotCount: '共 {0} 份',
        dockerPublished: 'Docker 发布端口',
        statusCard: '防火墙状态',
        statusCardTip: '运行状态与默认入站策略',
        rescueChannelTip: 'SSH、面板等关键端口始终放行，避免被锁在外面',
        rescuePortSwitchHelper:
            '开启后加入端口白名单；宽松模式下所有端口本就放行，该开关仅在白名单模式下产生拦截差异。',
        snapshotCardTip: '变更前留个还原点，可一键回滚',
        bootCheck: '开机自检',
        bootOk: '正常',
        rescuePanel: '面板',
        snapshotLatest: '最近备份',
        snapshotEmpty: '暂无快照',
        snapshotRestoreTarget: '将使用快照 {0} 覆盖当前 1Panel 防火墙链。',
        intentLabel: '意图',
        intentAllowPort: '放行端口',
        intentBlockPort: '封禁端口',
        intentBlockSource: '封禁来源 IP',
        intentAllowSource: '放行来源 IP',
        effectSummaryPort: '{0} {1} {2}，来自 {3}',
        effectSummaryAddr: '{0}来自 {1} 的入站访问',
        effectSummaryDocker: '（含 Docker 发布端口）',
        inboundEmpty: '暂无入站规则',
        inboundEmptyHint: '使用上方工具栏放行端口、封禁 IP 或创建规则。',
        sectionActions: '操作与确认',
        sectionActionsTip: '启停、初始化、白名单模式与待确认会话',
        warnNotReady: '防火墙尚未初始化',
        warnNotReadyHelper: '点击「初始化」配置保底端口与默认策略，规则在初始化后才会生效。',
        warnNotActive: '防火墙未启用',
        warnNotActiveHelper: '服务器当前没有入站防护，建议立即启动。',
        dockerProtection: 'Docker 发布端口防护',
        ipv6RulesCap: 'IPv6 规则',
        capSupported: '支持',
        capNotSupported: '不支持',
        pendingSessionLabel: '待确认会话',
        pendingSessionActive: '待确认（{0}s）',
        pendingSessionNone: '无',
        overviewHint: '5 秒看清有效状态与下一步安全动作；变更前建议先打快照，以便一键回滚。',
    },
    runtime: {
        runtime: '运行环境',
        workDir: '工作目录',
