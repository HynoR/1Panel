...
const message = {
    ...
    firewall: {
        ...
        destination: '目的地',
        flowSectionTitle: '入站評估順序',
        flowSectionHint: '自上而下匹配，命中即停止，未命中的走預設策略。',
        intentLabel: '意圖',
        intentAllowPort: '放行埠',
        intentBlockPort: '封鎖埠',
        intentBlockSource: '封鎖來源 IP',
        intentAllowSource: '放行來源 IP',
        effectSummaryPort: '{0} {1} {2}，來自 {3}',
        effectSummaryAddr: '{0}來自 {1} 的入站存取',
        effectSummaryDocker: '（含 Docker 發布埠）',
        inboundEmpty: '尚無入站規則',
        inboundEmptyHint: '使用上方工具列放行埠、封鎖 IP 或建立規則。',
        sectionActions: '操作與確認',
        sectionActionsTip: '啟停、初始化、白名單模式與待確認會話',
        warnNotReady: '防火牆尚未初始化',
        warnNotReadyHelper: '點擊「初始化」設定保底埠與預設策略，規則在初始化後才會生效。',
        warnNotActive: '防火牆未啟用',
        warnNotActiveHelper: '伺服器目前沒有入站防護，建議立即啟動。',
        dockerProtection: 'Docker 發布埠防護',
        ipv6RulesCap: 'IPv6 規則',
        capSupported: '支援',
        capNotSupported: '不支援',
        pendingSessionLabel: '待確認會話',
        pendingSessionActive: '待確認（{0}s）',
        pendingSessionNone: '無',
        overviewHint: '5 秒看清有效狀態與下一步安全動作；變更前建議先打快照，以便一鍵復原。',
        ...
        modeExternal: 'Manage ufw (no control over start/stop)',
        modeExternalTip:
        ...
        forwardEmpty:
            'No port-forwarding rules yet. Forward traffic from an external port to another internal IP / port.',
        overview: 'Overview',
        inboundRule: 'Inbound Rule | Inbound Rules',
        allowPort: 'Allow Port',
        blockIP: 'Block IP',
        level: 'Level',
        source: 'Source',
        ruleObjectPort: 'By port',
        ruleObjectAddress: 'By source IP',
        advancedOptions: 'Advanced options',
        goOverviewInit: 'Firewall is not ready yet. Go to Overview to start and initialize it.',
        goOverviewStart: 'Go to Overview to start',
        rescueChannel: "Rescue ports (won't lock you out)",
        rescueAllowed: 'Allowed',
        rescueClosable: 'Closable',
        rescueReadOnly: 'Rescue port (read-only)',
        defaultPolicy: 'Default inbound policy',
        policyLoose: 'Loose mode (unlisted ports allowed)',
        policyRecommended: 'Recommended',
        policyStrict: 'Whitelist mode (unlisted ports denied)',
        wizardStrictWarn:
            'Only listed ports will be accessible; all others are denied. Incorrect configuration may lock you out of the server.',
        ...
        flowInbound: 'Inbound',
        flowRescue: 'Rescue ports',
        flowDeny: 'Blocklist',
        flowBaseline: 'Rescue ports',
        flowAllow: 'Allowlist',
        flowDefaultLoose: 'Default: Loose mode',
        flowDefaultStrict: 'Default: Whitelist mode',
        flowInboundTip: 'Inbound traffic is matched top-down in the order below; the first match wins.',
        flowRescueTip: 'Temporarily allows your current connection so a mistaken block cannot lock you out.',
        flowDenyTip: 'The blocklist has the highest priority — a match is rejected immediately.',
        flowBaselineTip: 'Rescue ports (SSH, panel) are always allowed; cannot be deleted or flipped to block.',
        flowAllowTip: 'Allow rules you have opened manually.',
        flowDefaultLooseTip: 'Inbound traffic matching no rule is allowed by default.',
        flowDefaultStrictTip: 'Inbound traffic matching no rule is rejected by default.',
        strategyTip: 'Allow = permit access; Block = deny access. Rescue ports (SSH, panel) are always protected.',
        hintAllowPort:
            'Once allowed, the matched ports accept inbound access; with no source IP set they are open to all sources.',
        hintDenyPort:
            'Once blocked, the matched ports reject inbound access. Rescue ports such as SSH and the panel stay protected and cannot be locked out.',
        hintAllowAddress: 'Once allowed, inbound access from this IP / CIDR is permitted.',
        hintDenyAddress: 'Once blocked, all inbound access from this IP / CIDR is rejected.',
        levelDeny: 'Blocklist',
        levelBaseline: 'Rescue ports',
        levelAllow: 'Allowlist',
        noListen: 'No listener',
        riskTitle: 'This rule may lock you out',
        riskBlockSelf:
            'This rule may block your current connection; if you lose access, it auto-reverts in 60s unless confirmed.',
        riskAck: 'I understand the risk',
        riskProceed: 'Apply anyway',
        redlineTitle: 'Operation rejected',
        redlineBlockBoth:
            'This rule would block both SSH and the 1Panel console; rejected. To deny inbound by default, use the Whitelist mode switch.',
        redlineHint: 'To allow access, use Allow instead, or narrow the blocked range to a specific IP segment.',
        applying: 'Applying firewall changes…',
        autoReverted: 'Changes unconfirmed and auto-reverted',
        wizardStep1: 'Confirm rescue ports',
        wizardStep2: 'Choose default policy',
        wizardStep3: 'Apply & self-check',
        enableManaged: 'Enable 1Panel management',
        disableManaged: 'Disable 1Panel management',
        requiredPorts: 'Rescue ports (cannot be deleted)',
        editablePorts: 'Editable ports',
        deleteRescuePortHelper:
            'After deleting {0}, this port will no longer be auto-allowed by the firewall and external access may break. Continue?',
        snapshotCount: '{0} total',
        dockerPublished: 'Docker-published port',
        statusCard: 'Firewall status',
        statusCardTip: 'Runtime status and default inbound policy',
        rescueChannelTip: 'Key ports like SSH and the panel stay open so you are never locked out',
        rescuePortSwitchHelper:
            'After enabling, the port is added to the allowlist. In Loose mode all ports are already allowed, so this switch only changes blocking behavior in Whitelist mode.',
        snapshotCardTip: 'Keep a restore point before changes — roll back in one click',
        bootCheck: 'Boot self-check',
        bootOk: 'Normal',
        rescuePanel: 'Console',
        snapshotLatest: 'Last backup',
        snapshotEmpty: 'No snapshots',
        snapshotRestoreTarget: 'This will overwrite the current 1Panel firewall chains with snapshot {0}.',
    },
    runtime: {
        runtime: '執行環境',
        workDir: '工作目錄',
