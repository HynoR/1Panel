...
const message = {
    ...
    firewall: {
        ...
        confirmTitle: '確認防火牆變更',
        confirmTip: '已套用 {count} 項變更，若 {seconds} 秒內未確認將自動復原。',
        confirmKeep: '確認保留',
        revertNow: '立即復原',
        applyToDocker: '同時封鎖 Docker 發布的連接埠流量',
        applyToDockerHelper: '同步套用到 Docker 發布的連接埠（透過 DOCKER-USER），避免被 Docker 繞過。',
        modeManaged: '1Panel 全權管理',
        modeManagedTip: '1Panel 全權管理此主機的 iptables 規則。',
        modeExternal: '管理 ufw（不介入啟停）',
        modeExternalTip: '{0} 由系統管理，1Panel 代為操作，不修改其開機啟動行為。',
        conflictHelper: '偵測到 firewalld 與 ufw 同時執行，請停用其中之一以避免衝突。',
        bootDegraded: '防火牆開機自檢：{0}。部分規則可能未生效，請檢查設定。',
        quarantineView: '查看被隔離規則',
        quarantineTitle: '被隔離的舊版拒絕規則',
        quarantineHelper:
            '這些舊版拒絕規則在舊版配置中從未生效。升級時已隔離，避免啟用後將你鎖在伺服器外。本版本不支援單條還原；如需保留，請審核後手動新增拒絕規則。',
        quarantineEmpty: '沒有被隔離的規則',
        quarantineClean: '清除全部',
        quarantineCleanConfirm: '清除全部被隔離規則並隱藏降級提示？',
        snapshot: '快照',
        snapshotHelper: '快照會備份完整規則集；還原只重建 1Panel 鏈（保留 Docker 與其他規則），並進入提交確認視窗。',
        snapshotTag: '標籤',
        snapshotRestoreHelper: '還原將重建 1Panel 防火牆鏈並開啟確認視窗；若被鎖在外且未確認，將自動復原。是否繼續？',
        family: 'IP 版本',
        familyBoth: 'IPv4 + IPv6',
        forwardEmpty: '尚無連接埠轉發規則。可將外部連接埠流量轉發到內部 IP / 連接埠。',
        overview: '概覽',
        inboundRule: '入站規則',
        allowPort: '放行連接埠',
        blockIP: '封鎖 IP',
        ...
        source: '來源',
        ruleObjectPort: '按連接埠',
        ruleObjectAddress: '按來源 IP',
        advancedOptions: '進階選項',
        goOverviewInit: '防火牆尚未就緒，請前往「概覽」啟動並初始化。',
        goOverviewStart: '前往概覽啟動',
        rescueChannel: '保底連接埠（避免被鎖在外）',
        rescueAllowed: '已放行',
        rescueClosable: '可關閉',
        rescueReadOnly: '保底連接埠（唯讀）',
        defaultPolicy: '預設入站策略',
        policyLoose: '寬鬆模式（未列出連接埠預設放行）',
        policyRecommended: '推薦',
        policyStrict: '白名單模式（未列出連接埠預設拒絕）',
        wizardStrictWarn: '僅列出的連接埠可存取，其餘全部拒絕，設定不當可能讓你被鎖在伺服器外。',
        whitelistMode: '白名單模式',
        whitelistModeTip:
            '白名單模式：僅放行已列出的連接埠，其餘入站一律拒絕。啟用前請確認 SSH、面板及服務連接埠都已放行。',
        enableWhitelistTitle: '啟用白名單模式',
        enableWhitelistHelper:
            '啟用後，未在「入站規則」中放行的連接埠將全部被拒絕（SSH、面板等保底連接埠除外）。若放行規則不全可能導致服務無法存取；60 秒內未在面板點選「確認保留」將自動復原。是否繼續？',
        disableWhitelistTitle: '停用白名單模式',
        disableWhitelistHelper: '停用後恢復寬鬆模式：未列出的連接埠預設放行。是否繼續？',
        flowInbound: '入站',
        flowRescue: '保底連接埠',
        flowDeny: '封鎖清單',
        flowBaseline: '保底連接埠',
        flowAllow: '放行清單',
        flowDefaultLoose: '預設：寬鬆模式',
        flowDefaultStrict: '預設：白名單模式',
        flowInboundTip: '入站流量依下方順序由上而下比對，命中即停止。',
        flowRescueTip: '暫時放行你目前的連線，避免誤封後被鎖在外。',
        flowDenyTip: '封鎖清單優先順序最高，命中即拒絕。',
        flowBaselineTip: '保底連接埠（SSH、面板）永遠放行，不可刪除或改為封鎖。',
        flowAllowTip: '你手動開放的放行規則。',
        flowDefaultLooseTip: '未命中任何規則的入站流量預設放行。',
        flowDefaultStrictTip: '未命中任何規則的入站流量預設拒絕。',
        strategyTip: '放行 = 允許存取；封鎖 = 拒絕存取。SSH、面板等保底連接埠永遠受保護。',
        hintAllowPort: '放行後，命中的連接埠將允許入站存取；未填寫來源 IP 時會對所有來源開放。',
        hintDenyPort: '封鎖後，命中的連接埠將拒絕入站存取。SSH、面板等保底連接埠受保護，不會被誤鎖。',
        hintAllowAddress: '放行後，來自此 IP / CIDR 的入站存取將被允許。',
        hintDenyAddress: '封鎖後，來自此 IP / CIDR 的所有入站存取將被拒絕。',
        levelDeny: '封鎖清單',
        levelBaseline: '保底連接埠',
        levelAllow: '放行清單',
        noListen: '無監聽',
        riskTitle: '這條規則可能讓你失聯',
        riskBlockSelf: '此規則可能阻斷你目前的連線；若失去存取且未確認，60 秒後會自動復原。',
        riskAck: '我已了解風險',
        riskProceed: '仍要套用',
        redlineTitle: '操作已拒絕',
        redlineBlockBoth: '此規則會同時阻斷 SSH 與 1Panel 面板，已拒絕。如需預設拒絕入站，請使用「白名單模式」開關。',
        redlineHint: '如需放通請改用「放行」，或將封鎖範圍縮小到特定 IP 網段。',
        applying: '正在套用防火牆變更…',
        autoReverted: '變更未確認，已自動復原',
        wizardStep1: '確認保底連接埠',
        wizardStep2: '選擇預設策略',
        wizardStep3: '套用並自檢',
        enableManaged: '啟用 1Panel 管理',
        disableManaged: '停用 1Panel 管理',
        requiredPorts: '保底連接埠（不可刪除）',
        editablePorts: '可編輯連接埠',
        deleteRescuePortHelper: '刪除 {0} 後，此連接埠將不再隨防火牆自動放行，可能影響外部存取。是否繼續？',
        snapshotCount: '共 {0} 份',
        dockerPublished: 'Docker 發布連接埠',
        statusCard: '防火牆狀態',
        statusCardTip: '執行狀態與預設入站策略',
        rescueChannelTip: 'SSH、面板等關鍵連接埠會持續放行，避免被鎖在外',
        rescuePortSwitchHelper:
            '啟用後會加入連接埠白名單；寬鬆模式下所有連接埠原本就放行，此開關只會在白名單模式下改變封鎖行為。',
        snapshotCardTip: '變更前保留還原點，可一鍵復原',
        bootCheck: '開機自檢',
        bootOk: '正常',
        rescuePanel: '面板',
        snapshotLatest: '最近備份',
        snapshotEmpty: '尚無快照',
        snapshotRestoreTarget: '將使用快照 {0} 覆蓋目前的 1Panel 防火牆鏈。',
    },
    runtime: {
        runtime: '執行環境',
        workDir: '工作目錄',
